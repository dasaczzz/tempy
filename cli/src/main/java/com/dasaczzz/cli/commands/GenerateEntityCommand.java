...
public class GenerateEntityCommand {
    ...
    public String genEntity(
            @ShellOption(help = "Entity name in PascalCase (e.g. BlogPost)") String entityName,
            @ShellOption(
                    defaultValue = GenerateEntityUseCase.DEFAULT_BASE_PACKAGE,
                    help = "Base package of the backend (e.g. com.dasaczzz.tempy)"
            ) String basePackage,
            @ShellOption(
                    defaultValue = GenerateEntityUseCase.DEFAULT_BACKEND_PATH,
                    help = "Path to the backend module root (e.g. ../backend)"
            ) String backendPath) {
